/* global starsandstoriesIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'starsandstories-supports-dark-theme' ) ) {
	starsandstoriesDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function starsandstoriesDarkModeEditorInit( attempt ) {
	...
	if ( starsandstoriesIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						starsandstoriesDarkModeEditorInit( attempt + 1 );
					},
